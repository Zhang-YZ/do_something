...
csvPath = "./result_1.csv"

rawlogPaths = ["Hadoop","Linux"]
rawlogInfo = {}
for i in rawlogPaths:
    rawlogInfo[i] = {"size":os.path.getsize(dataPath+"2k"+i+"/rawlog.log"),"path":dataPath+i+"/rawlog.log","judge":0}
...
for path in algPaths:
    tempList0 = os.listdir(resultPath+path)
    tempList = [x for x in tempList0 if ("Hadoop" in x or "Linux" in x)]
    for subPath in tempList:
        tempPath = resultPath+path+"/"+subPath+"/matchResults.txt"
        sourceSize = rawlogInfo[subPath.split("_")[0]]["size"]
        try:
        ...
        for way in ways:
            tempargv=[way,rawlogInfo[subPath.split("_")[0]]["path"]]
            deal_argv(tempargv)
        for way in ways:
            tempargv=[way,tempPath]
            deal_argv_match(tempargv,rawlogInfo[subPath.split("_")[0]]["size"])
