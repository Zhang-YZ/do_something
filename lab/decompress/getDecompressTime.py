...
for path in rawlogPaths:
    ...
    try:
        with open(tempBasePath+logTemplatesName,"r") as logFile: 
            logTemplates = logFile.readlines()
        with open(tempBasePath+matchResultsName,"r") as matchFile:
        ...
        with open(tempBasePath+rawlogName,"w") as rawlogFile:
        ...
        result.append([temp,"--","--","--","--","--","--"])
        continue
    result.append([temp,round(time1-time0,5),round(time3-time2,5),round(time4-time3,5),round(time5-time4,5),round(time6-time5,5),round(time7-time6,5),round(oneTime1-oneTime0,5)])

with open("uncompress_result.csv","w") as resultFile:
    file_csv = csv.writer(resultFile)
    title = ["dataset","ft-tree_time","lzma_time","gzip_time","bzip2_time","7zip_time","zip_time","ft-tree_one_log_time"]  
