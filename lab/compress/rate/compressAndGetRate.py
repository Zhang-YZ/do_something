...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        ...
        with open(csvPath,"w") as f:
            ...
            if "first" in wayargvs:
                row.extend(["lzma","lzma_rate","gz","gz_rate","bz2","bz2_rate","7z","7z_rate","zip","zip_rate"])
            if "second" in wayargvs:
                row.extend(["double_lzma","double_lzma_rate","double_gz","double_gz_rate","double_bz2","double_bz2_rate","double_7z","double_7z_rate","double_zip","double_zip_rate"]) 
            f_csv.writerow(row)
